...
unsafe fn set_handler(handler: SigHandler) -> Result<()> {
    ...
    sigaction(Signal::SIGTERM, &a).context("failed to sigaction")?;
    sigaction(Signal::SIGQUIT, &a).context("failed to sigaction")?;
    sigaction(Signal::SIGINT, &a).context("failed to sigaction")?;
    sigaction(Signal::SIGHUP, &a).context("failed to sigaction")?;
    sigaction(Signal::SIGUSR1, &a).context("failed to sigaction")?;
    sigaction(Signal::SIGUSR2, &a).context("failed to sigaction")?;
    Ok(())
}
...
pub fn _to_signal(signal: &str) -> Result<Signal> {
    Ok(match signal {
        ...
        _ => panic! {"{} is not a valid signal", signal},
    })
}

pub fn signal_process<T: Into<Option<Signal>>>(pid: Pid, signal: T) -> Result<()> {
    kill(pid, signal)?;
    Ok(())
}
...
pub fn raise_for_parent(signal: Signal) -> Result<()> {
    ...
    if signal != Signal::SIGKILL && signal != Signal::SIGSTOP {
        let a = SigAction::new(SigHandler::SigDfl, SaFlags::empty(), SigSet::all());
        unsafe {
            sigaction(signal, &a).context("failed to sigaction")?;
        }
    }
    // make sure the signal is unblocked
    let mut s = SigSet::empty();
    s.add(signal);
    s.thread_unblock().context("failed to unblock signal")?;
    // raise the signal
    raise(signal).context(format!("failed to raise signal {:?}", signal))?;
    Ok(())
}
